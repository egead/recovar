...
class KfoldDynamicTrainer(KfoldTrainer):
    ...
    def _compute_loss_from_outputs(self, x, f1, f2, f3, f4, f5, y1, y2, y3, y4, y5):
        ...
        return tf.reduce_mean(per_sample_recon_loss + per_sample_ensemble_loss)

    def _train_on_selected(self, model, optimizer, x_selected):
        """Forward+backward with gradient accumulation over chunks to avoid OOM."""
        chunk_size = BATCH_SIZE
        n_chunks = int(np.ceil(len(x_selected) / chunk_size))
        accumulated_grads = None
        total_loss = 0.0

        for ci in range(n_chunks):
            x_chunk = x_selected[ci * chunk_size : (ci + 1) * chunk_size]

            with tf.GradientTape() as tape:
                f1, f2, f3, f4, f5, y1, y2, y3, y4, y5 = model(x_chunk, training=True)
                chunk_loss = self._compute_loss_from_outputs(
                    x_chunk, f1, f2, f3, f4, f5, y1, y2, y3, y4, y5
                ) / n_chunks

            grads = tape.gradient(chunk_loss, model.trainable_variables)
    ...
    def _train_one_epoch(self, model, optimizer, train_gen, classifier, epoch):
        ...
        for super_batch_idx in range(n_super_batches):
            ...
            loss = self._train_on_selected(model, optimizer, x_selected)
    ...
    def _validate(self, model, validation_gen):
        ...
        for batch_idx in range(n_batches):
            ...
            loss = self._compute_loss_from_outputs(
                x, f1, f2, f3, f4, f5, y1, y2, y3, y4, y5
            )
