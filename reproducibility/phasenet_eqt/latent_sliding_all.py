...
WINDOW_SIZE = 3000
SAMPLING_RATE = 100
START_TIME = datetime(2025, 11, 10, 2, 40)
BATCH_SIZE = 1024
...
LATENT_START_TIME = datetime(2025, 11, 10, 2, 40)
LATENT_END_TIME = datetime(2025, 11, 10, 2, 45)
...
for npy_file in npy_files:
    ...
    for i in range(0, waveform.shape[0] - WINDOW_SIZE + 1, stride_samples):
        ...
        if len(batch) == BATCH_SIZE:
            ...
            # Check for samples within the time range and save plots as needed
            if SAVE_LATENT_PLOTS:
                ...
                for idx, meta in enumerate(batch_meta):
                    sample_time = meta['timestamp']

                    in_time_range = True
                    if latent_start is not None and sample_time < latent_start:
                        in_time_range = False
                    if latent_end is not None and sample_time > latent_end:
                        in_time_range = False
                    ...
                    if last_latent_save_time is None:
                        ...
                        time_since_last_save = (sample_time - last_latent_save_time).total_seconds()
                        if time_since_last_save >= LATENT_PLOT_INTERVAL:
                            should_save = True

                    if should_save:
                        samples_to_plot.append(idx)
                        last_latent_save_time = sample_time
                ...
                if samples_to_plot:
                    ...
                    print(f"\nSaved {len(samples_to_plot)} latent plots (last at {batch_meta[samples_to_plot[-1]]['timestamp'].strftime('%H:%M:%S')})")
